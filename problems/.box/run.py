...
def run_solution(sol_fn, input_fn,
    ...
    file_limit=file_limit*1024
    int_time_limit = int(round(time_limit))

    #import sys
    #print('mem_limit',mem_limit,file=sys.stderr)
    #print('cur mem_limit',resource.getrlimit(resource.RLIMIT_DATA),file=sys.stderr)
    #print('file_limit',file_limit,file=sys.stderr)

    pid = os.fork()
    if pid == 0:
        resource.setrlimit(resource.RLIMIT_CPU, (int_time_limit+1, int_time_limit + 1))
    ...
    if result.running_time > time_limit:
        ...
        if checker == 'diff':
            ...
            # cms style, use diff -b
            if subprocess.call(['diff', "-b", reference_fn, output_fn],
            ...
            if subprocess.call([checker, input_fn, reference_fn, output_fn],
                               stdout = NamedTemporaryFile(),
                               stderr = NamedTemporaryFile()) == 0:
                result.status = 'AC'
            else:
