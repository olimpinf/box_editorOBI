...
\newcommand{\explaincat}[1]{explain-#1.tex}
\newcounter{count}
...
\whiledo{\value{count}<10}{
  ...
  \bf{\small Exemplo de entrada \thecount}
  \insereArquivo{\incat{\thecount}}
  \end{minipage}
  &
  \begin{minipage}[t]{0.5\textwidth}
  %\bf{\small Sample output \thecount}
  \bf{\small Exemplo de saída \thecount}
  ...
  \IfFileExists{\CWD/\explaincat{\thecount}} {
    \begin{minipage}[c]{0.95\textwidth}
      \emph{Explicação do exemplo \thecount:}
    \input{\CWD/\explaincat{\thecount}}
  \end{minipage}\\[12pt]
  } % leave next line empty
...
\newcommand{\inputdescline}[1]{\inputdesc{A entrada consiste de uma única linha que contém {#1}}}
...
\newcommand{\outputdescline}[2]{\outputdesc{Seu programa deve produzir {#1} representando {#2}}}
...
\newcommand{\outputfloatnotice}[1]{O resultado deve ser escrito como um número racional
  com exatamente {#1} dígitos após o ponto decimal, arredondado se necessário.}
